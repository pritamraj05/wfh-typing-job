...
import { manualApprovePayment } from "./actions";

export default function ManualApproveForm() {
  ...
  const handleApprove = async (e: React.FormEvent) => {
    ...
    try {
      const res = await manualApprovePayment(email);
      if (res.error) {
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-8">
      <h3 className="text-lg font-bold mb-4">Manual Offline Payment Approval</h3>
      <p className="text-sm text-gray-600 mb-4">Enter user's email to manually approve their ₹1500 security deposit.</p>
